...
def _auto_turn_radius_hint(prev: ScanBlockLine, nxt: ScanBlockLine) -> u.Quantity:
    ...
    start_x, start_y = margin_stop_of(prev)
    stop_x, stop_y = margin_start_of(nxt)
    start_unit = str(start_x.unit)
    start = get_quantity(
        np.array(
            [
                start_x.to_value(start_unit),
                start_y.to_value(start_unit),
            ],
            dtype=float,
        ),
        unit=start_unit,
    ...
    stop = get_quantity(
        np.array(
            [
                stop_x.to_value(start_unit),
                stop_y.to_value(start_unit),
            ],
            dtype=float,
        ),
        unit=start_unit,
